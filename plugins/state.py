...
def sort_aircraft(aircraftinbounds, idx, n_aircraft):
    ...
    for j in aircraftinbounds:
        tcpa, tinhor, touthor, dcpa, conflict = calc_tdcpa(idx, j)
        tcpa = min(tcpa, 600)
        temp = [j,tcpa,dcpa,conflict,tinhor,touthor]
        ac_tdcpa.append(temp)
    ac_tdcpa = np.array(ac_tdcpa)
    
    if len(ac_tdcpa) > 0:
        ac_tdcpa = ac_tdcpa[ac_tdcpa[:,1]>=0]
...
def calc_tdcpa(own_idx, int_idx):
    ...
    return tcpa, tinhor, touthor, dcpa, conflict
