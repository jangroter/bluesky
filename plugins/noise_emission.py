...
class Noisepollution(core.Entity):
    def __init__(self):
        ...
        self.schiphol = [4.760783,52.3068953] # lat,lon coords of schiphol for reference to x_array and y_array
    ...
    def update(self, dt):
        for i in range(traf.ntraf):
            ...
            x = np.cos(np.radians(brg))*dist*nm
            y = np.sin(np.radians(brg))*dist*nm
            z = bs.traf.alt[i]
            ...
            self.noise[i] += np.sum(self.pop_array * sound)
