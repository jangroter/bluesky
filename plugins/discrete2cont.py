...
def init_plugin():
    ...
    discrete2cont = Discrete2cont()
    
    # Configuration parameters
    config = {
        # The name of your plugin
        'plugin_name':     'DISCRETE2CONT',
...
class Discrete2cont(core.Entity):  
    def __init__(self):
        super().__init__()
    ...
    def check_ac(self):
        if bs.traf.ntraf == 0 and self.index < self.indexmax:
            ...
            for lat,lon in zip(self.waypoints['lat'].iloc[self.index][1:],self.waypoints['lon'].iloc[self.index][1:]):
    ...
    def check_done(self,idx):
        ...
        fuel = self.get_rew_fuel(idx,coeff = -0.12/17.)
        ...
        if done:
            ...
            np.savetxt('fuel.csv',self.fuel_array)
            np.savetxt('noise.csv',self.noise_array)
            bs.traf.delete(idx)
    ...
    def get_reward(self,idx,state,state_):
        ...
        fuel = self.get_rew_fuel(idx, coeff = -0.12/17.) #Fuel ~ Noise * 17 
